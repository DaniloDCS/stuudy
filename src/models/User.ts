...
class User implements IUser {
  ...
  constructor({ id, userId, avatar, username, name, birthday, biography, email, phone, admin, accesses, network, createdAt, updatedAt }: IUser) {
    this.id = id || uuid();
    this.userId = userId;
  ...
  public removeNetwork = (network: Network): void => {
    this.network = this.network.filter((item: Network) => item.id !== network.id);
...
class Accesses implements IAccesses {
  ...
  constructor({ id, date, location, exit, machine, browser, duration }: IAccesses) {
    this.id = id || uuid();
...
class Network {
  ...
  constructor({ id, url, visualization, visits, createdAt, updatedAt }: { id: string, url: string, visualization?: boolean, visits?: number, createdAt?: Date, updatedAt?: Date }) {
    this.id = id || uuid();
    this.url = url;
...
class Users {
  ...
  public getUser = (id: string): User | null => this.users.find(user => user.getId() === id) || null;

  public getUserByUsername = (username: string): User | null => this.users.find(user => user.getUsername() === username) || null;

  public getUserByEmail = (email: string): User | null => this.users.find(user => user.getEmail() === email) || null;

  public getUserByPhone = (phone: string): User | null => this.users.find(user => user.getPhone() === phone) || null;
