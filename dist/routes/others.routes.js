...
class OthersRoutes {
  ...
  constructor() {
   ...
   routes() {
    ...
    this.router.get('/dashboard', _authenticate.authenticate.call(void 0, ).auth, async (req, res) => {
      const user = new (0, _User.User)(_App2.default.get("user"));

      const { data, error: erroNotifies } = await _connection.connection.from('Notifies').select('*').match({ userId: user.getId() });
      ...
      const notifies = new (0, _Notify.Notifies)({
        userId: user.getId(),
        notifies: _optionalChain([data, 'access', _ => _[0], 'optionalAccess', _2 => _2.notifies, 'access', _3 => _3.map, 'call', _4 => _4((notify) => new (0, _Notify.Notify)(notify))]) || []
      });
    ...
    this.router.get('/u/:username', async (req, res) => {
      ...
      const { data: searchFriend, error: errorSearchFriend } = await _connection.connection.from("Friends").select("id, from:sender(*), to:recipient(*)").eq("recipient", user.getId()).eq("sender", me.getId());
      if (errorSearchFriend) return res.status(500).json({ message: 'Erro ao buscar amigos', errorSearchFriend });

      const { data: searchFriend2, error: errorSearchFriend2 } = await _connection.connection.from("Friends").select("id, from:sender(*), to:recipient(*)").eq("recipient", me.getId()).eq("sender", user.getId());
      ...
      if (searchFriend.length > 0 || searchFriend2.length > 0) {
        ...
        const { data: sendRequest, error: errorFriendRequest } = await _connection.connection.from("Solicitations").select("*, from:senderId(*), to:recipientId(*)").eq("recipientId", me.getId()).eq("senderId", user.getId());
        if (errorFriendRequest) return res.status(500).json({ message: 'Erro ao buscar solicitações de amizade', error: errorFriendRequest });

        const { data: reciveRequest2, error: errorFriendRequest2 } = await _connection.connection.from("Solicitations").select("*, from:senderId(*), to:recipientId(*)").eq("recipientId", user.getId()).eq("senderId", me.getId());
    ...
    this.router.get('/me', _authenticate.authenticate.call(void 0, ).auth, async (req, res) => {
    ...
    this.router.get('/chat', _authenticate.authenticate.call(void 0, ).auth, async (req, res) => {
      const user = new (0, _User.User)(_App2.default.get("user"));
